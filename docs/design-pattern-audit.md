...
| 7 | **Inconsistent error path between `ExternalToolWrapper` and `StreamingTool.execute`** | `src/minimal_harness/tool/wrapper.py:153–158`, `src/minimal_harness/tool/base.py:90–115`, `src/minimal_harness/agent/openai.py:191–210` | Have `ExternalToolWrapper` raise a custom `ToolExecutionError`. Catch it inside `StreamingTool.execute` for a uniform error string. | A single, uniform error path simplifies debugging and ensures the LLM receives consistent tool-result formatting. |
| 8 | **`__ALL__` typo breaks `from module import *`** | `src/minimal_harness/llm/__init__.py:11`, `src/minimal_harness/__init__.py:11` | Rename to `__all__` (lowercase). | Star imports currently expose internal names that may not be intended for public API, leading to namespace pollution and backward-compatibility hazards. |
| 9 | **Configuration constants scattered and hardcoded** | `src/minimal_harness/client/built_in/config.py:18–26`, `src/minimal_harness/llm/openai.py:21`, `src/minimal_harness/agent/openai.py:36` | Introduce a single `Settings` class (e.g., Pydantic `BaseSettings`) that reads from environment variables with these strings as fallbacks. | Centralized configuration prevents drift, makes deployment easier, and allows operators to override behavior without editing source code. |
| 10 | **Unsafe `cast` calls for `TypedDict` construction** | `src/minimal_harness/agent/openai.py` (lines 64, 95, 117) | Define typed factory functions (e.g., `user_message(...)`, `tool_message(...)`) instead of `cast`. | Factories give both type safety and a single point of validation. They also make the intent explicit. |
| 11 | **Duplicated OpenAI client construction logic** | `test/test_client.py:111–126`, `examples/relation_digging/main.py:17–24` | Add a factory in `minimal_harness.llm.openai` (e.g., `create_openai_client(api_key, base_url)`). | DRY. Centralizes default handling and makes it easier to add retry logic or logging later. |
| 12 | **`ToolRegistrationProtocol` is an incomplete contract** | `src/minimal_harness/tool/base.py` (lines 26–32) | Update the protocol to match the real `ToolRegistry.register_external_tool` signature, including `script_path` and `**kwargs`. | Protocols are documentation as much as they are type constraints. An incomplete protocol undermines the purpose of structural subtyping. |
| 13 | **Magic number truncation in TUI display** | `src/minimal_harness/client/built_in/app.py` (lines 461, 482) | Define a named constant: `MAX_DISPLAY_LENGTH = 500`. | Named constants are self-documenting and prevent magic numbers from proliferating. |
| 14 | **Tests write artifacts to the repository root** | `test/test_client.py` (lines 171, 186, 201, 216, etc.) | Use pytest's `tmp_path` fixture for all output files. | Keeps the working directory clean, prevents flaky tests from leftover files, and avoids race conditions in parallel test runs. |
| 15 | **`ChatInput` bypasses Textual's declarative action framework** | `src/minimal_harness/client/built_in/widgets.py` (lines 42–69) | Move `action_submit` bindings to the `App` level and use Textual's `BINDINGS` system. | Using the framework's routing makes key behavior configurable and testable without simulating low-level key events. |
