...
```python
from minimal_harness import CompactionSettings
...
The summarizer is built internally from the runtime's LLM provider
resolver (see `minimal_harness.agent._compaction.build_summarizer`).
...
The summarizer is built from the runtime's LLM provider resolver
(via `build_summarizer`); the threshold and `keep_recent` come from
the session's owning agent's `CompactionSettings`, falling back to
the runtime's `default_compaction_settings`. This is the same path
