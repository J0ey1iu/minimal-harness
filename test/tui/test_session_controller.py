...
class TestSessionCreation:
    def test_create_session_returns_valid_session(self, controller):
        ...
        mem = controller._ctx.memory_store.get_memory(session.memory_id)
    ...
    def test_consecutive_creates_new_memory(self, controller):
        s1 = controller.create_session(agent_name="agent_a")
        m1 = controller._ctx.memory_store.get_memory(s1.memory_id)
        s2 = controller.create_session(agent_name="agent_b")
        m2 = controller._ctx.memory_store.get_memory(s2.memory_id)
        assert m1 is not m2
...
class TestSessionManagement:
    ...
    def test_get_memory_returns_current_session_memory(self, controller):
        controller.create_session(agent_name="agent_a")
        memory = controller.get_memory()
        assert memory is not None
        assert memory.memory_id == controller.current_session.memory_id

    def test_get_memory_none_when_no_session(self, controller):
        assert controller.get_memory() is None


class TestRunManagement:
    def test_start_run_calls_runtime_with_ids(self, controller):
        ...
        result = controller.start_run(session, "hello world")
        assert result is not None

        controller._runtime.run.assert_called_once()
        call_args = controller._runtime.run.call_args
    ...
    def test_end_run_removes_from_active(self, controller):
        session = controller.create_session(agent_name="primary")
        controller._runtime.run.return_value = (
            MagicMock(spec=asyncio.Task),
            asyncio.Event(),
...
class TestGetAllSessionsMetadata:
    def test_sessions_metadata_includes_memory_session(self, controller):
